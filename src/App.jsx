...
export default function App() {
  ...
  // --- REUSABLE COMPONENTS ---
  const SearchBar = () => (
    <div className="flex items-center gap-2 mb-4 w-full px-4 pt-4 shrink-0">
      <div className="flex-1 bg-gray-800 rounded-xl shadow-lg flex items-center px-4 py-3 border border-gray-700">
        <Search size={20} className="text-gray-400" />
        <input 
          type="text" placeholder="ค้นหาทรัพย์สิน, รหัสทรัพย์..." 
  ...
  const AdminView = () => {
    ...
    return (
      <div className="p-4 pb-6 overflow-y-auto h-full bg-gray-900 w-full">
        <div className="mb-4">
      ...
      <div className="p-6 flex flex-col justify-center h-full bg-gray-900 pb-6 overflow-y-auto w-full">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="flex justify-end">
            <button type="button" onClick={() => setCurrentView('forgot-password')} className="text-sm text-blue-400 hover:text-blue-300 transition-colors">ลืมรหัสผ่าน?</button>
          </div>
        ...
        <div className="mt-6 flex items-center justify-between">
          <hr className="w-full border-gray-700" /><span className="px-3 text-gray-500 text-sm whitespace-nowrap">หรือเข้าสู่ระบบด้วย</span><hr className="w-full border-gray-700" />
        </div>
  ...
  const ForgotPasswordView = () => {
    ...
    return (
      <div className="p-6 flex flex-col justify-center h-full bg-gray-900 pb-6 overflow-y-auto w-full">
        <div className="mb-8">
          <button onClick={() => setCurrentView('login')} className="text-gray-400 hover:text-white mb-6 flex items-center"><ChevronLeft size={20} className="mr-1" /> กลับ</button>
        ...
        <form onSubmit={handleResetPassword} className="space-y-6">
          <div className="relative">
            ...
            <input type="email" required placeholder="กรอกอีเมลของคุณ" disabled={isSending} className="w-full bg-gray-800 border border-gray-700 text-white placeholder-gray-500 rounded-xl py-3 pl-10 pr-4 outline-none focus:border-blue-500 disabled:opacity-50" value={resetEmail} onChange={e => setResetEmail(e.target.value)} />
          </div>
          <button type="submit" disabled={isSending} className="w-full bg-blue-600 text-white rounded-xl py-3 font-bold shadow-lg hover:bg-blue-700 flex justify-center items-center disabled:bg-blue-800">
  ...
  // --- RENDER MAIN LAYOUT ---
  return (
    <div className="flex flex-col h-[100dvh] w-full max-w-md mx-auto bg-gray-900 relative shadow-2xl overflow-hidden font-sans">
      
      {/* Header - Fixed Height */}
      {currentView !== 'detail' && currentView !== 'login' && currentView !== 'forgot-password' && currentView !== 'register' && (
        <header className="bg-gray-900 pt-10 pb-4 px-4 shadow-sm z-10 flex justify-between items-center border-b border-gray-800 shrink-0">
          <div className="flex items-center text-white shrink-0 min-w-0">
            <SaimaiLogo />
            <h1 className="text-lg sm:text-xl font-extrabold tracking-tight truncate">บ้านสายไหม</h1>
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            ...
            {!currentUser && currentView !== 'login' && (
              <button onClick={() => setCurrentView('login')} className="text-[10px] sm:text-xs font-bold text-blue-400 bg-blue-900/30 border border-blue-800 px-2 sm:px-3 py-1.5 rounded-full hover:bg-blue-900/50 whitespace-nowrap">
                เข้าสู่ระบบ
              </button>
            )}
          </div>

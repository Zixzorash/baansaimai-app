...
const LineIcon = ({ className = "w-6 h-6" }) => (
  <svg viewBox="0 0 24 24" className={className} fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    <path d="M24 10.304c0-5.232-5.383-9.5-12-9.5-6.617 0-12 4.268-12 9.5 0 4.69 4.263 8.605 10.023 9.332.39.083.92.257 1.053.59.12.302.08.775.038 1.08l-.164 1.02c-.045.285-.24 1.087 1.103.593 1.266-.465 6.83-4.02 9.317-6.885 1.714-1.895 2.63-3.794 2.63-5.73zm-15.82 3.66h-1.875a.41.41 0 0 1-.41-.41V9.224a.41.41 0 0 1 .41-.41h.41a.41.41 0 0 1 .41.41v3.916h1.065a.41.41 0 0 1 .41.41v.41c0 .226-.184.41-.41.41zm3.32 0a.41.41 0 0 1-.41-.41v-.41c0-.226.184-.41.41-.41h.01c.214 0 .386-.165.41-.378V9.224a.41.41 0 0 1 .41-.41h.41a.41.41 0 0 1 .41.41v4.32a.41.41 0 0 1-.41.41h-1.25zm5.556 0a.41.41 0 0 1-.41-.41v-2.31l-1.616-2.023a.41.41 0 0 1-.035-.453.411.411 0 0 1 .369-.23h.41c.15 0 .288.083.359.215l1.107 1.543V9.224a.41.41 0 0 1 .41-.41h.41a.41.41 0 0 1 .41.41v4.32a.41.41 0 0 1-.41.41h-.41a.41.41 0 0 1-.359-.215l-1.107-1.543v2.091c0 .226-.184.41-.41.41zm3.834 0a.41.41 0 0 1-.41-.41V9.224a.41.41 0 0 1 .41-.41h1.875a.41.41 0 0 1 .41.41v.41a.41.41 0 0 1-.41.41h-1.465v.93h1.465a.41.41 0 0 1 .41.41v.41a.41.41 0 0 1-.41.41h-1.465v.93h1.465a.41.41 0 0 1 .41.41v.41a.41.41 0 0 1-.41.41h-1.875z" />
  </svg>
);
...
function MainApp() {
      ...
      filteredProperties.forEach(prop => {
        ...
        const badgeHtml = `
          <div style="background-color: #1f2937; border: 2px solid ${prop.type === 'rent' ? '#3b82f6' : '#ef4444'}; border-radius: 12px; padding: 4px 8px; text-align: center; box-shadow: 0 4px 10px rgba(0,0,0,0.5); width: max-content; transform: translate(-50%, -100%);">
            ...
            <div style="font-size: 13px; color: ${prop.type === 'rent' ? '#60a5fa' : '#ef4444'}; font-weight: 800;">${prop.price.toLocaleString()} ฿</div>
          </div>
        `;
    ...
    return (
      <div className="flex flex-col h-full bg-gray-900 overflow-y-auto pb-6 w-full relative">
        ...
        <div className="flex-1 p-5 md:p-8 md:max-w-4xl md:mx-auto w-full bg-gray-900 -mt-6 md:-mt-10 rounded-t-3xl relative z-10 border-t border-gray-800 shadow-2xl">
          ...
          {/* --- ย้ายปุ่ม โทร, LINE, เส้นทาง มาไว้ตรงนี้ (ใต้ราคา) --- */}
          <div className="mt-4 mb-6 grid grid-cols-3 gap-3 max-w-lg mx-auto">
  ...
  const LoginView = () => {
    ...
    // โหลด Google Sign-In SDK แบบ Dynamic Script เพื่อตัดปัญหาความขัดข้องของ Build Node Module 
    useEffect(() => {
      ...
      const initGoogleLogin = () => {
        if (window.google?.accounts?.id) {
          ...
          window.google.accounts.id.renderButton(
            ...
            { theme: "filled_black", size: "large", width: 384 }
          );
        }
      };
    ...
    return (
      <div className="p-6 flex flex-col justify-center h-full bg-gray-900 w-full animate-in fade-in duration-300">
        <div className="max-w-sm mx-auto w-full mb-8">
          {/* --- เพิ่มปุ่ม Back สำหรับหน้า Login --- */}
          <button onClick={() => navigate(previousView || 'map')} className="text-gray-400 hover:text-white mb-6 flex items-center transition-colors">
        ...
        <form onSubmit={onSubmit} className="space-y-4 max-w-sm mx-auto w-full">
          ...
          <button type="submit" disabled={isLoading} className="w-full bg-blue-600 text-white rounded-xl py-3 font-bold flex justify-center items-center active:scale-95 transition-transform">{isLoading && loginMethod === 'normal' && <Loader2 size={20} className="animate-spin mr-2" />} เข้าสู่ระบบ</button>
        </form>
        ...
        <div className="mt-6 space-y-3 max-w-sm mx-auto w-full">
          {/* ส่วนล็อคอินด้วย Google แบบ Direct SDK */}
